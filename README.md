...
void ler_dados_conta(struct Conta *conta, const char *tipo_conta) {
...
void escrever_dados_conta(FILE *arquivo, const char *tipo_conta, struct Conta *conta) {
...
void escrever_dados_cliente(FILE *arquivo, struct Cliente *cliente) {
...
void consultar_dados_cliente(const char *numero_conta) {
    printf("\n=========== Consulta de Dados de Conta ===========\n");
...
// Função para alterar dados de conta
...
// Função para alterar dados de funcionário
...
// Função para alterar dados de cliente
...
// Função do menu para alterar dados
...
// Função para mostrar o extrato e salvar em um arquivo CSV
void mostrar_extrato() {
    char senha_cliente[20];
    int senha_correta = 0;

    // Pedir a senha do cliente
    ...
    if (senha_correta) {
        // Exibir e salvar extrato em CSV
        float saldo = 0.0;
        char linha[100];
        FILE *arquivo_extrato;
        FILE *arquivo_saldo;
        ...
        arquivo_extrato = fopen("extrato.txt", "r");
        FILE *arquivo_csv = fopen("extrato.csv", "w");
        if (arquivo_extrato == NULL) {
            printf("\n ========= EXTRATO ========\n");
            printf("Não houve movimentações bancárias.\n");
            printf("\nSaldo: R$ %.2f\n", saldo);
            printf("==========================================\n");
            fprintf(arquivo_csv, "Não houve movimentações bancárias.\nSaldo: R$ %.2f\n", saldo);
            fclose(arquivo_csv);
            return;
        }

        printf("\n ========= EXTRATO ========\n");
        fprintf(arquivo_csv, "Tipo de Movimentação;Valor\n");
        while (fgets(linha, sizeof(linha), arquivo_extrato) != NULL) {
            printf("%s", linha);
            fprintf(arquivo_csv, "%s", linha);
        }
        printf("\nSaldo: R$ %.2f\n", saldo);
        printf("==========================================\n");
        fprintf(arquivo_csv, "\nSaldo: R$ %.2f\n", saldo);
        fclose(arquivo_extrato);
        fclose(arquivo_csv);
        printf("Extrato salvo em extrato.csv\n");
    } else {
        printf("Senha incorreta.\n");
    }
}
void consultar_limite() {
    ...
    if (senha_correta) {
        if (strcmp(tipo_de_conta, "Corrente") == 0) {
            ...
            printf("Data de vencimento do limite: %s\n", conta.data_de_vencimento);
            printf("==========================================\n");
        } else {
            printf("A conta não é do tipo corrente e não possui limite.\n");
        }
    } else {
        printf("Senha incorreta.\n");
    }
...
void menuCliente(struct Cliente clientes[], int num_clientes) {
    char senha[20];
    char cpf_cliente[20];
    int opcao;
    ...
    if (strcmp(senha, "senha_cliente") == 0) {
        printf("Acesso como cliente concedido.\n");
        printf("Digite o CPF do cliente: ");
        scanf("%s", cpf_cliente);
        do {
            printf("\n============= Menu Cliente =============\n");
	    ...
	    do {
	        ...
	        switch (opcao) {
	            case 1:
	                ...
	                scanf("%s", senha);
	                if (strcmp(senha, "senha_funcionario") == 0) {
	                    menuFuncionario();
	                } else {
	                    printf("Senha incorreta!\n");
	                }
	                break;
	            case 2:
	                menuCliente(clientes, num_clientes);
	                break;
	            case 3:
	                printf("Saindo...\n");
	                break;
